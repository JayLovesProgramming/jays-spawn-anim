...
local function startSpawnCamera(coordsWhereYouSpawn)
    ...
    if Config.Debug then print("Destroyed Cam 2") end
    Wait(1300)
    freeze = false
    inProcessOfMovingCam = false
...
function startDisablingControlsPlz()
    CreateThread(function()
        ...
        while inProcessOfMovingCam do
            Wait(0)
            if Config.DebugAdvanced then print("Started disabling controls") end
            if freeze then FreezeEntityPosition(PlayerPedId(), true) end
        ...
        Wait(1000)
        inProcessOfMovingCam = false
        if Config.Debug then print("Stopped disabling controls") end
        FreezeEntityPosition(PlayerPedId(), false)
    end)
end

RegisterNetEvent("jays-spawn-anim:doSpawnAnimationAndSpawnIn", function(selectedSpawnCoords) startSpawnCamera(selectedSpawnCoords) end)
CreateThread(function()
    if Config.Debug then -- THESE ARE ONLY DEBUG COMMANDS ///// MAKE SURE TO DISABLE IN NORMAL ENVIROMENT!!!! -- 
        RegisterCommand("startstartanim", function()
            local selectedSpawnCoords = GetEntityCoords(PlayerPedId()) -- COMMAND 1 -- Eventually callback from server the required coords
            TriggerEvent("jays-spawn-anim:doSpawnAnimationAndSpawnIn", selectedSpawnCoords)
        end)

        RegisterCommand("testmainanim", function()
            playerHeadBone = GetPedBoneIndex(PlayerPedId(), "SKEL_HEAD") -- COMMAND 2
            startingCam = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
